...
class ModelRunner:
    ...
    def __init__(self, rank: int, config: Config):
        ...
        # Resolve DP isolation: when VLLM_DEVICE_CONTROL_ENV_VAR_PLACEHOLDER
        # is set, ATOM manages DP at the EngineCore level (gloo), so each
        # DP rank's ModelRunners should form an independent NCCL world with
        # TP only — no NCCL DP groups.  The env var value encodes the
        # physical GPU indices for this DP rank (e.g. "2,3").
        dp_rank_local = config.parallel_config.data_parallel_rank_local or 0
        num_gpus = torch.cuda.device_count()
        placeholder_devices = os.environ.get("VLLM_DEVICE_CONTROL_ENV_VAR_PLACEHOLDER")
        dp_isolated = placeholder_devices is not None

        if dp_isolated:
            device_indices = [int(x) for x in placeholder_devices.split(",")]
        ...
        # Pre-set HIP_VISIBLE_DEVICES to all GPUs so that aiter's
        # init_distributed_environment does not restrict visibility to
        # only world_size (=TP) devices, which would hide DP rank 1+ GPUs.
    ...
    def allocate_kv_cache(self, num_kvcache_blocks):
        ...
        # Synchronize within the TP group only.  Each DP rank manages its
        # own KV cache independently via its Engine Core, so a global
        # barrier here would deadlock when DP ranks execute
        # resume_memory at different times (one DP rank's ModelRunners
        # wait at the barrier while the other DP rank's Engine Core is
        # blocked in _sync_dp_state, preventing its ModelRunners from
        # ever reaching allocate_kv_cache).
